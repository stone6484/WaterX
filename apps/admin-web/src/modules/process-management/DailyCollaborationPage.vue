...
async function load(){const ticket=++epoch,api=client();detail.value=null;lines.value=[];records.value=[];cells.value={};historyTemplate.value=[];historical.value=false;historyLabel.value='';assignee.value='';reviewer.value='';note.value='';notice.value='';category.value='全部';search.value='';dirty.value=false;blocked.value=true;busy.value=true;error.value='';try{const values=await api.processDailyContext();if(ticket!==epoch)return;lines.value=values;if(props.initialLineId)lineId.value=props.initialLineId;lineId.value=values.some(l=>l.id===lineId.value)?lineId.value:values[0]?.id||'';if(lineId.value){const rows=await api.processDailyList(lineId.value);if(ticket!==epoch)return;records.value=rows;if(props.initialRecordId){if(!rows.some(r=>r.id===props.initialRecordId))throw new Error('该任务不在当前人员与工艺线的可见范围');const record=await api.processDailyDetail(props.initialRecordId);if(ticket!==epoch)return;apply(record)}}blocked.value=false}catch(e){if(ticket===epoch)error.value=(e as Error).message}finally{if(ticket===epoch)busy.value=false}}
