...
const detailTabs = [
  { id:'overview', name:'指标详情' }, { id:'standard', name:'评分标准' },
  { id:'interpretation', name:'结果解读' }, { id:'data', name:'数据与事实' }
] as const
const detailId = useId()
const tableColumns = ['指标','实际值','单位','适用基线','偏差','得分','状态','数据说明','操作']
const tableBody=ref<HTMLElement|null>(null), tableGutter=ref(0)
function measureTable(){if(tableBody.value)tableGutter.value=tableBody.value.offsetWidth-tableBody.value.clientWidth}
let tableObserver:ResizeObserver|undefined
onMounted(()=>{tableObserver=new ResizeObserver(measureTable);if(tableBody.value)tableObserver.observe(tableBody.value);measureTable()})
onBeforeUnmount(()=>tableObserver?.disconnect())
watch(()=>props.activePage,()=>nextTick(measureTable))
...
function onDetailTabKeydown(event: KeyboardEvent) {
  const index = detailTabs.findIndex(tab => tab.id === detailTab.value)
  let next = index
  ...
  nextTick(() => document.getElementById(`${detailId}-${detailTab.value}`)?.focus())
}
const riskMeta = {
...
<template>
  <section class="mq-page">
    ...
    <WxTableSurface class="mq-metric-panel">
      <div class="mq-table-x-scroll" tabindex="0" role="region" :aria-label="`${currentPage.title}表格，可左右滚动`"><div class="mq-table-stack">
        <div class="mq-table-header" :style="{paddingRight:`${tableGutter}px`}"><WxTable class="mq-table mq-table-head" aria-hidden="true"><colgroup><col v-for="column in tableColumns" :key="column"></colgroup><thead><tr><th v-for="column in tableColumns" :key="column">{{column}}</th></tr></thead></WxTable></div>
        <div ref="tableBody" class="mq-table-body" tabindex="0" role="region" :aria-label="`${currentPage.title}记录，可上下滚动`"><WxTable class="mq-table" :aria-label="`${currentPage.title}指标`"><colgroup><col v-for="column in tableColumns" :key="column"></colgroup><thead class="mq-sr-only"><tr><th v-for="column in tableColumns" :key="column" scope="col">{{column}}</th></tr></thead><tbody>
    ...
    <WxDialog :open="!!selectedMetric" placement="right" :label="selectedMetric ? `${selectedMetric.code} ${selectedMetric.name}指标详情` : '指标详情'" @close="closeMetric">
      <aside v-if="selectedMetric" class="mq-drawer">
        <header>
          ...
          <button type="button" aria-label="关闭指标详情" @click="closeMetric">×</button>
        </header>
        <nav role="tablist" aria-label="指标详情分区" @keydown="onDetailTabKeydown"><button v-for="tab in detailTabs" :id="`${detailId}-${tab.id}`" :key="tab.id" type="button" role="tab" :aria-selected="detailTab===tab.id" :aria-controls="`${detailId}-panel`" :tabindex="detailTab===tab.id ? 0 : -1" :class="{selected:detailTab===tab.id}" @click="detailTab=tab.id">{{tab.name}}</button></nav>
        <div :id="`${detailId}-panel`" class="mq-drawer-body" role="tabpanel" :aria-labelledby="`${detailId}-${detailTab}`" tabindex="0">
        ...
        <footer><span>样例水厂 · 固定可重置数据</span><div><button type="button" @click="closeMetric">关闭</button><button v-if="selectedMetric.score!==null && selectedMetric.score<selectedMetric.maxScore" type="button" class="primary" @click="startImprovement(selectedMetric)">发起改进</button></div></footer>
      </aside>
    </WxDialog>
  </section>
</template>
...
/* Dimension completion is not a risk verdict and must not change severity when selected. */
.mq-score-dock .mq-dimension-card i b{background:var(--wx-blue-700)}
...
.mq-table col:nth-child(1){width:13%}.mq-table col:nth-child(2){width:14%}.mq-table col:nth-child(3){width:7%}.mq-table col:nth-child(4){width:11%}.mq-table col:nth-child(5){width:9%}.mq-table col:nth-child(6){width:8%}.mq-table col:nth-child(7){width:9%}.mq-table col:nth-child(8){width:12%}.mq-table col:nth-child(9){width:17%}
...
.mq-table-header{border-top:var(--wx-border-subtle);border-bottom:var(--wx-border-subtle);background:var(--wx-n25)}
.mq-table-head th{height:42px;border-top:0;border-bottom:0}
.mq-table td{overflow-wrap:anywhere}
...
.mq-unit{color:var(--wx-n600);white-space:normal}.mq-deviation-value{font-variant-numeric:tabular-nums;white-space:normal}
.risk-normal .mq-deviation-value{color:var(--wx-success-strong)}.risk-attention .mq-deviation-value{color:var(--wx-warning-strong)}.risk-risk .mq-deviation-value{color:var(--wx-danger-strong)}.risk-unavailable .mq-deviation-value{color:var(--wx-n500)}
...
/* Engineering 1.1: retain the approved table; complete the detail interaction. */
.mq-sr-only{position:absolute;width:1px;height:1px;margin:-1px;padding:0;overflow:hidden;clip-path:inset(50%);white-space:nowrap}
.mq-page button:focus-visible,.mq-drawer button:focus-visible,.mq-table-x-scroll:focus-visible,.mq-table-body:focus-visible,.mq-drawer-body:focus-visible{outline:2px solid var(--wx-blue-600);outline-offset:2px}
.mq-drawer{width:min(680px,94vw);height:100%;max-height:100%;min-height:0;padding:0;grid-template-rows:auto auto minmax(0,1fr) auto;background:var(--wx-n0);color:var(--wx-n700);box-shadow:var(--wx-shadow-overlay)}
.mq-drawer>header{min-height:104px;padding:20px 22px;border-bottom:var(--wx-border-subtle);background:var(--wx-n0)}
.mq-drawer>header p{font-size:11px;color:var(--wx-blue-700)}
.mq-drawer>header h2{font-size:18px;color:var(--wx-n800);overflow-wrap:anywhere}
.mq-drawer>header span,.mq-drawer>header em{font-size:10px;border-radius:var(--wx-radius-sm)}
.mq-drawer>header em{background:var(--wx-warning-bg);color:var(--wx-warning-strong)}
.mq-drawer>header>button{flex:none;width:32px;height:32px;padding:0;border-radius:var(--wx-radius-sm);color:var(--wx-n600)}
.mq-drawer>header>button:hover{background:var(--wx-n50)}
.mq-drawer>nav{padding:6px 16px;border-bottom:var(--wx-border-subtle)}
.mq-drawer>nav button{height:34px;font-size:12px;border-radius:var(--wx-radius-sm);color:var(--wx-n600);text-align:center}
.mq-drawer>nav button:hover{background:var(--wx-n50)}
.mq-drawer>nav button.selected{background:var(--wx-info-bg);color:var(--wx-blue-700);font-weight:600}
.mq-drawer-body{min-height:0;gap:0;padding:0 16px 16px;overscroll-behavior:contain;scrollbar-width:thin;scrollbar-color:rgba(96,123,137,.16) transparent}
.mq-drawer-body::-webkit-scrollbar{width:1px}.mq-drawer-body::-webkit-scrollbar-thumb{background:rgba(96,123,137,.16);border-radius:999px}
.mq-detail-card,.mq-pending,.mq-result-callout,.mq-fact-detail{min-width:0;padding:18px 0;border:0;border-bottom:var(--wx-border-subtle);border-radius:0;background:transparent;overflow-wrap:anywhere}
.mq-detail-card:last-child,.mq-pending:last-child,.mq-result-callout:last-child,.mq-fact-detail:last-child{border-bottom:0}
.mq-detail-card h3,.mq-fact-detail h3{font-size:14px;color:var(--wx-n800)}
.mq-detail-card p,.mq-pending p,.mq-result-callout p,.mq-fact-detail p,.mq-detail-card dd,.mq-fact-detail dd{font-size:12px;line-height:1.7;color:var(--wx-n700)}
.mq-detail-card dt,.mq-fact-detail dt{font-size:12px;color:var(--wx-n500)}
.mq-pending b{font-size:12px;color:var(--wx-warning-strong)}
.mq-draft-warning{margin-top:16px;padding:12px;border:1px solid var(--wx-warning-border);border-radius:var(--wx-radius-sm);background:var(--wx-warning-bg)}
.mq-draft-warning b{font-size:12px;color:var(--wx-warning-strong)}.mq-draft-warning span{font-size:11px;color:var(--wx-n600);line-height:1.6}
.mq-formula{background:var(--wx-n25);border-radius:var(--wx-radius-sm);color:var(--wx-n700)!important}
.mq-detail-score{gap:0;padding:18px 0;border-bottom:var(--wx-border-subtle)}
.mq-detail-score div{min-width:0;min-height:0;padding:0 12px 0 0;border:0;border-radius:0;background:transparent;overflow-wrap:anywhere}
.mq-detail-score span,.mq-result-callout span{font-size:11px;color:var(--wx-n500)}.mq-detail-score strong{font-size:14px;color:var(--wx-n800)}
.mq-result-callout strong{color:var(--wx-n800)}
.mq-fact-list button{background:var(--wx-n25);border:var(--wx-border-default);border-radius:var(--wx-radius-sm)}
.mq-fact-list button.selected{background:var(--wx-info-bg);border-color:var(--wx-blue-600)}
.mq-fact-list span,.mq-fact-list small,.mq-fact-detail>header span,.mq-fact-detail>header em{font-size:11px}.mq-fact-list b{font-size:12px;color:var(--wx-n700)}
.mq-history-bars b,.mq-history-bars small{font-size:10px}.mq-history-bars i{background:var(--wx-blue-500);border-radius:3px 3px 0 0}
.mq-drawer>footer{min-height:60px;gap:12px;padding:12px 16px;border-top:var(--wx-border-subtle);background:var(--wx-n0)}
.mq-drawer>footer span{font-size:11px;color:var(--wx-n500)}
.mq-drawer>footer button{height:var(--wx-control-height);min-width:72px;padding:0 12px;border:var(--wx-border-default);border-radius:var(--wx-radius-sm);font-size:12px;background:var(--wx-n0);color:var(--wx-blue-700);white-space:nowrap;cursor:pointer}
.mq-drawer>footer button:hover{background:var(--wx-n25);border-color:var(--wx-blue-600)}
.mq-drawer>footer button.primary{background:var(--wx-blue-700);border-color:var(--wx-blue-700);color:var(--wx-n0)}
.mq-drawer>footer button.primary:hover{background:var(--wx-deep-600);border-color:var(--wx-deep-600)}
@supports selector(::-webkit-scrollbar){.mq-drawer-body,.mq-table-body,.mq-table-x-scroll{scrollbar-width:auto;scrollbar-color:auto}}
@media(max-width:760px){.mq-detail-score{row-gap:16px}.mq-detail-card dl div,.mq-fact-detail dl div{grid-template-columns:80px minmax(0,1fr);gap:8px}.mq-drawer>footer{flex-wrap:wrap}.mq-drawer>footer>div{margin-left:auto}}
</style>
