...
import { WxButton, WxTabs, WxDialog } from '../../components/waterx'
...
const currentAuditRecord=computed(()=>view.value==='form'?form.value:selectedRecord.value)
...
<template>
  <section class="lab-raw-module">
    <template v-if="view==='library'">
      <div class="lab-library-controls"><WxTabs class="lab-template-tabs" aria-label="原始记录分类"><button :class="{selected:category==='全部'}" :aria-pressed="category==='全部'" @click="category='全部'">全部 18</button><button v-for="item in labCategories" :key="item" :class="{selected:category===item}" :aria-pressed="category===item" @click="category=item">{{item}} {{labTemplates.filter(t=>t.category===item).length}}</button></WxTabs><div class="lab-raw-summary"><span>共 {{labTemplates.length}} 类</span><span>待复核 {{statusCounts.review}}</span><span>已锁定 {{statusCounts.locked}}</span></div></div>
    ...
    <template v-else-if="view==='list'">
      <section class="lab-raw-toolbar"><div><WxButton @click="view='library'">← 返回项目</WxButton><WxButton variant="primary" @click="newRecord">＋ 新建记录</WxButton><WxButton @click="editRecord()">编辑</WxButton><WxButton :disabled="selectedRecord?.status!=='待复核'" @click="reviewSelected">复核通过</WxButton><WxButton :disabled="selectedRecord?.status!=='已复核'" @click="lockSelected">锁定归档</WxButton><WxButton :disabled="!selectedRecord?.locked" @click="beginCorrection">发起更正</WxButton><WxButton variant="danger" @click="deleteSelected">删除</WxButton></div><div><b>{{activeTemplate.code}} · {{activeTemplate.shortName}}</b><span :class="['method-state',activeTemplate.methodState==='现行方法'?'active':activeTemplate.methodState==='项目方法'?'project':'pending']">{{activeTemplate.methodState}}</span></div></section>
      <section v-if="activeTemplate.scopeNote" class="lab-method-warning"><b>适用性说明</b><span>{{activeTemplate.scopeNote}}</span></section>
      <section class="lab-record-filter"><label>关键词<input v-model="keyword" placeholder="编号、样品、检测人或复核人" /></label><label>记录状态<select v-model="statusFilter"><option value="">全部状态</option><option>草稿</option><option>待复核</option><option>已复核</option><option>已锁定</option><option>更正中</option></select></label><WxButton @click="keyword='';statusFilter=''">清空</WxButton><WxButton @click="resetDemo">重置示范数据</WxButton></section>
      <section class="lab-record-table-wrap" role="region" aria-label="原始记录台账，可横向和纵向滚动" tabindex="0"><table class="lab-record-table"><colgroup><col style="width:190px"><col style="width:58px"><col style="width:150px"><col style="width:104px"><col style="width:190px"><col style="width:72px"><col style="width:76px"><col style="width:84px"><col style="width:84px"><col style="width:148px"><col style="width:144px"></colgroup><thead><tr><th>记录编号</th><th>版本</th><th>样品</th><th>检测日期</th><th>方法/公式版本</th><th>质控</th><th>状态</th><th>检测人</th><th>复核人</th><th>更新时间</th><th>操作</th></tr></thead><tbody><tr v-for="record in activeRecords" :key="record.id" :class="{selected:selectedRecord?.id===record.id}" @click="selectedId=record.id" @dblclick="openFormRecord(record)"><td><code>{{record.recordNo}}</code></td><td>V{{record.version}}</td><td><b>{{record.sampleName}}</b><small>{{record.sampleSource}}</small></td><td>{{record.testDate}}</td><td><small>{{activeTemplate.methodVersion}}</small><small>{{activeTemplate.formulaVersion}}</small></td><td><span :class="['qc-chip',record.qcStatus]">{{record.qcStatus}}</span></td><td><span :class="['record-status',statusClass(record.status)]">{{record.status}}</span></td><td>{{record.analyst}}</td><td>{{record.reviewer||'—'}}</td><td>{{record.updatedAt}}</td><td><WxButton @click.stop="openFormRecord(record)">{{record.locked?'查看':'打开'}}</WxButton><WxButton @click.stop="selectedId=record.id;showAudit=true">留痕</WxButton></td></tr><tr v-if="!activeRecords.length"><td colspan="11" class="empty">暂无符合条件的原始记录</td></tr></tbody></table></section>

    </template>

    <template v-else-if="form">
      <section class="lab-raw-toolbar lab-form-actions"><div><WxButton @click="view='list'">← 返回台账</WxButton><WxButton v-if="!form.locked" variant="primary" @click="saveRecord()">保存草稿</WxButton><WxButton v-if="!form.locked" :disabled="formQc.status==='阻断'||formQc.status==='待完成'" @click="submitReview">提交复核</WxButton><WxButton @click="printRecord">打印 / 预览</WxButton><WxButton @click="showAudit=true">查看留痕</WxButton></div><div><span :class="['record-status',statusClass(form.status)]">{{form.status}}</span><span :class="['qc-chip',formQc.status]">质控 {{formQc.status}}</span><span>V{{form.version}}</span></div></section>
      ...
      <section class="lab-raw-paper-wrap">
        <article class="lab-paper lab-raw-paper">
          ...
          <table class="lab-paper-meta-table"><tbody><tr><th>记录编号</th><td><input aria-label="记录编号" v-model="form.recordNo" :disabled="form.locked" /></td><th>样品来源</th><td><input aria-label="样品来源" v-model="form.sampleSource" :disabled="form.locked" /></td><th>样品名称</th><td><input aria-label="样品名称" v-model="form.sampleName" :disabled="form.locked" /></td></tr><tr><th>取样日期</th><td><input aria-label="取样日期" v-model="form.sampleDate" type="date" :disabled="form.locked" /></td><th>检测日期</th><td><input aria-label="检测日期" v-model="form.testDate" type="date" :disabled="form.locked" /></td><th>室温 / 湿度</th><td><input aria-label="室温" v-model="form.roomTemperature" :disabled="form.locked" /> ℃ / <input aria-label="湿度" v-model="form.humidity" :disabled="form.locked" /> %</td></tr><tr><th>仪器编号</th><td><input aria-label="仪器编号" v-model="form.instrumentNo" :disabled="form.locked" /></td><th>试剂批号</th><td><input aria-label="试剂批号" v-model="form.reagentBatch" :disabled="form.locked" /></td><th>状态</th><td>{{form.status}}</td></tr></tbody></table>
          ...
          <table class="lab-paper-sign"><tbody><tr><th>备注</th><td colspan="5"><textarea aria-label="备注" v-model="form.notes" :disabled="form.locked" rows="2"></textarea></td></tr><tr><th>检测人</th><td><input aria-label="检测人" v-model="form.analyst" :disabled="form.locked" /></td><th>复核人</th><td><input aria-label="复核人" v-model="form.reviewer" disabled /></td><th>锁定时间</th><td>{{form.lockedAt||'—'}}</td></tr><tr v-if="form.correctionReason"><th>更正原因</th><td colspan="5">{{form.correctionReason}}</td></tr></tbody></table>
          <footer>说明：黄色区域为人工录入的一手观测值，蓝灰区域为受控公式自动结果；打印件统一转为黑白并保留方法、公式、签字和版本信息。</footer>
        </article>
      </section>
    </template>

    <WxDialog :open="showAudit&&Boolean(currentAuditRecord)" label="原始记录审计留痕" placement="right" @close="showAudit=false"><section v-if="currentAuditRecord" class="lab-dialog-panel lab-audit-panel"><header><div><b>原始记录审计留痕</b><span>{{currentAuditRecord.recordNo}} · V{{currentAuditRecord.version}}</span></div><WxButton variant="ghost" aria-label="关闭留痕" @click="showAudit=false">×</WxButton></header><ol><li v-for="event in [...currentAuditRecord.audit].reverse()" :key="event.id"><time>{{event.at}}</time><div><b>{{event.action}}</b><span>{{event.operator}}</span><p>{{event.detail}}</p></div></li></ol><footer><WxButton @click="showAudit=false">关闭</WxButton></footer></section></WxDialog>
    <WxDialog :open="showCorrection" label="发起更正" :close-on-backdrop="false" :close-on-escape="false" @close="showCorrection=false"><form class="lab-dialog-panel lab-correction" @submit.prevent="createCorrection"><header><div><b>发起更正</b><span>锁定原版本不变，系统创建下一版本继续编辑。</span></div><WxButton type="button" variant="ghost" aria-label="关闭更正" @click="showCorrection=false">×</WxButton></header><label>更正原因<textarea v-model="correctionReason" rows="5" required placeholder="说明错误位置、原因和更正依据"></textarea></label><footer><WxButton type="button" @click="showCorrection=false">取消</WxButton><WxButton variant="primary" type="submit">创建更正版本</WxButton></footer></form></WxDialog>
    <WxDialog :open="showReview&&Boolean(selectedRecord)" label="复核原始记录" :close-on-backdrop="false" :close-on-escape="false" @close="showReview=false"><form class="lab-dialog-panel lab-correction" @submit.prevent="confirmReview"><header><div><b>复核原始记录</b><span>{{selectedRecord?.recordNo}} · 检测人 {{selectedRecord?.analyst}}</span></div><WxButton type="button" variant="ghost" aria-label="关闭复核" @click="showReview=false">×</WxButton></header><label>复核人<input v-model="reviewName" required placeholder="复核人必须与检测人不同" /></label><section class="lab-review-confirm"><b>复核确认</b><p>已核对一手观测值、方法与公式版本、自动计算结果、质控状态和异常说明。</p></section><footer><WxButton type="button" @click="showReview=false">取消</WxButton><WxButton variant="primary" type="submit">确认复核通过</WxButton></footer></form></WxDialog>
  </section>
</template>
